...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        truncate=document[:length]
        mask_len=random.randint(int(length*(1/8)), int(length*(3/8)))
        ind=random.randint(0,(length-mask_len))
        prefix=truncate[0:ind]
        mask=truncate[ind:(ind+mask_len)]
        suffix=truncate[(ind+mask_len):]
        ...
        ouput=masked_string[1:]
        x = torch.tensor([self.stoi[char] for char in input], dtype=torch.long)
        y = torch.tensor([self.stoi[char] for char in ouput], dtype=torch.long)
        return x, y
