...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    p = softmax(outsideVectors.dot(centerWordVec)).reshape((outsideVectors.shape[0], 1))
    loss = -np.log(p[outsideWordIdx])
    gradCenterVec = -outsideVectors[outsideWordIdx] + np.sum(outsideVectors * p, axis=0)
    gradOutsideVecs = p.dot(centerWordVec.reshape((outsideVectors.shape[1], 1)).T)
...
def negSamplingLossAndGradient(
    ...
    p = outsideVectors[indices].dot(centerWordVec).reshape((-1, 1))
    p1 = sigmoid(p[0])
    p2 = sigmoid(-p[1:])
    loss = -np.log(p1) - np.sum(np.log(p2), axis=0)
    gradCenterVec = outsideVectors[outsideWordIdx] * (p1 - 1) + \
                    np.sum(outsideVectors[negSampleWordIndices] * (1 - p2), axis=0)
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = centerWordVec * (p1 - 1)
    gradOutsideVecs[negSampleWordIndices] += centerWordVec * (1 - p2)
    indexCount = np.bincount(indices).reshape(-1, 1)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for w in outsideWords:
        l, gc, go = word2vecLossAndGradient(
            centerWordVectors[word2Ind[currentCenterWord]], word2Ind[w], outsideVectors, dataset)
        loss += l
        gradCenterVecs += gc
