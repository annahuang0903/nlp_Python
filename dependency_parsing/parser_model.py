...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(torch.Tensor(self.embed_size * self.n_features, self.n_features))
        self.embed_to_hidden_bias = nn.Parameter(torch.Tensor(1, self.n_features))
        nn.init.xavier_uniform(self.embed_to_hidden_weight)
        nn.init.uniform(self.embed_to_hidden_bias)
        self.dropout = nn.Dropout(p=self.dropout_prob)
        self.hidden_to_logits_weight = nn.Parameter(torch.Tensor(self.n_features, n_classes))
        self.hidden_to_logits_bias = nn.Parameter(torch.Tensor(1, n_classes))
